...
// AsyncRPCOperation_sendmany used as base
// todo: remove anything non z-transaction related
//

#include "asyncrpcoperation_createraw.h"
...
int findoutput(UniValue obj, int n) {
...
AsyncRPCOperation_createraw::AsyncRPCOperation_createraw(
...
void AsyncRPCOperation_createraw::main() {
    if (isCancelled())
        return;
...
bool AsyncRPCOperation_createraw::main_impl() {
    ...
    while (!vpubNewProcessed) {
        ...
        if (jsChange > 0) {
            changeOutputIndex = findoutput(obj, 1);
        }
    }
...
bool AsyncRPCOperation_createraw::find_utxos(bool fAcceptCoinbase=false) {
...
bool AsyncRPCOperation_createraw::find_unspent_notes() {
...
UniValue AsyncRPCOperation_createraw::perform_joinsplit(AsyncJoinSplitInfo & info) {
    std::vector<boost::optional < ZCIncrementalWitness>> witnesses;
    uint256 anchor;
    {
        LOCK(cs_main);
...
UniValue AsyncRPCOperation_createraw::perform_joinsplit(AsyncJoinSplitInfo & info, std::vector<JSOutPoint> & outPoints) {
    std::vector<boost::optional < ZCIncrementalWitness>> witnesses;
    uint256 anchor;
    {
        LOCK(cs_main);
...
UniValue AsyncRPCOperation_createraw::perform_joinsplit(
        AsyncJoinSplitInfo & info,
        std::vector<boost::optional < ZCIncrementalWitness>> witnesses,
        uint256 anchor)
{
...
void AsyncRPCOperation_createraw::add_taddr_outputs_to_tx() {
...
void AsyncRPCOperation_createraw::add_taddr_change_output_to_tx(CAmount amount) {
...
boost::array<unsigned char, ZC_MEMO_SIZE> AsyncRPCOperation_createraw::get_memo_from_hex_string(std::string s) {
 ...
 */
UniValue AsyncRPCOperation_createraw::getStatus() const {
